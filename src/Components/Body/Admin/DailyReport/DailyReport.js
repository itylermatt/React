...
class DailyReport extends  Component{
    componentDidMount() {
        ...
        axios.get(urls.url("users"),{params:{token:token}}).then(res=>{
            let users = [];
            let clicksCount = [];
    ...
    handleDateChange = (e)=>{
        ...
        // let token = ls.get('token');
        // this.setState({token:token});
        axios.post(urls.url("users"),{date : e.target.value, token:this.state.token}).then(res=>{
            let users = [];
            let clicksCount = [];
            let clicksPercentage = [];
            let undetectedCount = [];
            ...
            clicksCount.map((ret, count)=>{
                let percent = (((res.data.totalDetections - clicksCount[count])/ res.data.totalDetections)*100).toFixed(2);
                let item = res.data.totalDetections - clicksCount[count] ;
                if(isNaN(percent))
                    percent = 0 ;
                if(isNaN(item))
                    item = 0;
                undetectedCount.push(item);
                undetectedPercentage.push(percent);
                return null;
            });
